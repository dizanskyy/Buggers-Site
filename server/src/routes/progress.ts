...
router.get('/stats', authMiddleware, (req: AuthRequest, res) => {
  try {
    ...
    const user = db.prepare('SELECT id, username, xp, level, streak, role, avatar FROM users WHERE id = ?').get(userId) as any;
    ...
    for (const item of completedLessons as any[]) {
...
router.post('/complete', authMiddleware, (req: AuthRequest, res) => {
  try {
    ...
    const act = db.prepare('SELECT id, count, xp_earned FROM daily_activity WHERE user_id = ? AND date = ?').get(userId, today) as any;
    if (act) {
    ...
    if (count >= 1) awardAch('first_step');
    if (count >= 5) awardAch('quick_learner');
    if (count >= 15) awardAch('code_addict');
    if (count >= 30) awardAch('mastery_seeker');
    if (count >= 50) awardAch('grandmaster');

    if (trackId === 'gta-mp' && count >= 3) awardAch('gta_modder');
    if (trackId === 'cybersecurity' && count >= 3) awardAch('cyber_ninja');
    if (trackId === 'gamedev' && count >= 3) awardAch('game_architect');
    if (trackId === 'backend' && count >= 3) awardAch('backend_sorcerer');
    if (trackId === 'frontend' && count >= 3) awardAch('frontend_wizard');
...
// Toggle Bookmark
router.post('/bookmark', authMiddleware, (req: AuthRequest, res) => {
  try {
    const userId = req.userId;
    const { lessonId, trackId } = req.body;

    const existing = db.prepare('SELECT id FROM user_bookmarks WHERE user_id = ? AND lesson_id = ?').get(userId, lessonId) as any;
    if (existing) {
